# darshan_reops
HEP-CCE/SOP darshan reops analysis

## Checkout the repo
 1. Clone the repository without checking out files automatically
   ```
   git clone --filter=blob:none --no-checkout https://github.com/hep-cce2/IOSwork
   cd IOSwork
   git sparse-checkout init --cone
   git sparse-checkout set darshan_reops
   git checkout main
   ```
## Enviroment
 - Tested with `python3.12` + ROOT `6-36-04` + `pydarshan 3.5.0`
## Run
 - Identify and extract the reops of a given file
   ```
   python plot_offset_histogram.py <darshan_record> --include_names <filename>
   ```
 - Identify and extract the reops of a given file and map them to ROOT object
   1. dump the ROOT clusters or branchs/fields of a given TTree/RNTuple, or a list of TTree/RNTuple, or all existing ones in ROOT file
      ```
      python dump_cluster_boundary.py <paths-to-inputRootFile> <EventTreeName> <max_workers> <per_branch=0/1> <output=dump_{surfix}.json/dump_per_branch_{surfix}.json>
      ```
      `EventTreeName`: `NAME` or `NAME1,NAME2` or `all`
   2. map the offsets
      ```
      python plot_offset_histogram.py <darshan_record> --include_names inputRootFile  --enable_mapping --tree_branch_file <dump_{surfix}.json/dump_per_branch_{surfix}.json>
      ```
      In case of multiple collections, add `--conllection NAME/NAME1,NAME2/all`
      
   Note: The mapping checks if `per_branch_` is part of the `tree_branch_file` name to decide how to map the records and create the plots.
